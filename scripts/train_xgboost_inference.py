...
if not all_rows: raise RuntimeError('XGBOOST_NO_CANDLE_TRAINING_DATA')
data=pd.concat(all_rows).reset_index(drop=True); split=int(len(data)*0.8); X=data[FEATURES]; y=data['target'];
if y.nunique()<2: raise RuntimeError('XGBOOST_SINGLE_CLASS_LABELS')
...
results={}
for symbol,payload in market.get('symbols',{}).items():
    rows=(payload.get('candles') or {}).get('candles',[]); df=pd.DataFrame(rows)
