...
# Keep the public report contract (120 M1 / 30 M5), but fetch enough M1
# history for the DARTS normality window instead of silently truncating it.
        ...
        for raw in requested:
            ...
            if re.fullmatch(r'[A-Z]{6}', base_name) and base_name[:3] in fx_codes and base_name[3:] in fx_codes:
                ...
                if name not in cleaned:
                    cleaned.append(name)
        if not cleaned:
            raise RuntimeError('NO_VALID_FX_SYMBOLS')
        return cleaned
...
# Direct per-symbol recovery is the authoritative path for long history.
for symbol in symbols:
    for interval, key, target in ((60, 'm1', REQUEST_M1), (300, 'm5', REQUEST_M5)):
        try:
            direct = get('/api/market/candles?' + urllib.parse.urlencode({
                'symbol': symbol, 'interval': interval, 'count': target}), timeout=90, attempts=3)
            rows = direct.get('candles') or []
            if isinstance(rows, list) and rows:
                collected[symbol][key] = {
                    'candles': rows, 'source': direct.get('source'), 'symbol': symbol,
                    'interval_seconds': interval, 'read_only': True,
                }
            if len(rows) < (MIN_M1 if key == 'm1' else MIN_M5):
                errors[f'short:{symbol}:{interval}'] = f"{len(rows)}<{MIN_M1 if key == 'm1' else MIN_M5}"
        except Exception as exc:
...
out = {'source': base, 'read_only': True, 'health': health,
       'assets': [], 'symbols': {}, 'fetch_mode': 'per_symbol_direct_long_history',
...
print('railway_market_per_symbol=OK', len(fresh), 'fresh_of', len(symbols), 'm1_target', REQUEST_M1)
