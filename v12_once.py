...
def buscar_todos_pares():
    ...
    script = (
        ...
        "ok, _ = iq.connect()\n"
        "if not ok: print(json.dumps({'pares_otc':[],'pares_forex':[],'velas':{},'payouts':{}})); exit()\n"
        "time.sleep(1)\n"
        "otc_abertos=[]; forex_abertos=[]\n"
        "payouts={}\n"
        "try:\n"
        ...
        "    if v and len(v) >= 10:\n"
        "      result_m5[par] = [{'o':x['open'],'c':x['close'],'h':x['max'],'l':x['min'],'t':x['from']} for x in v]\n"
        "  except: pass\n"
