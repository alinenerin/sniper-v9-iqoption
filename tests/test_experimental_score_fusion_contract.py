...
class ExperimentalFusionContract(unittest.TestCase):
    ...
    def test_no_score_mode_bypasses_only_score_gate(self):
        m1, m3 = candles(), []
        low = SimpleNamespace(score=10, probability=.10, anomaly_score=20)
        with patch.dict("os.environ", {"NO_SCORE_MODE": "1"}):
            decision = select_timeframe(m1, m3, low, None)
        self.assertEqual(decision["selected"], "M1")
        # Deterministic lane does not reject solely for a low weighted score.
        rows = {"H4": {"direction": "CALL"}, "H1": {"direction": "CALL"},
                "M15": {"direction": "CALL"}, "M5": {"direction": "PUT"}}
        result = deterministic_confluence(rows, "CALL", 10.0)
        self.assertTrue(result["approved"])
        self.assertTrue(result["score_gate_bypassed"])
        self.assertFalse(deterministic_confluence(rows, "CALL", 99.0, ["M1:stale"])["approved"])
