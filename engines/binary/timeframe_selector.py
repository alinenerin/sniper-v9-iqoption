"""Seleção adaptativa M1/M3 para o operacional binário (somente leitura)."""
from __future__ import annotations
import os
from typing import Any, Dict


def _signal_threshold(is_otc: bool) -> float:
    key = "OTC_SIGNAL_THRESHOLD" if is_otc else "BINARY_SIGNAL_THRESHOLD"
    try:
        return float(os.environ.get(key, "70"))
    except (TypeError, ValueError):
        return 70.0


def _quality(candles) -> float:
    try:
        rows = list(candles or [])[-30:]
        if len(rows) < 10:
            return 0.0
        ranges = [max(float(r['high']) - float(r['low']), 1e-12) for r in rows]
        bodies = [abs(float(r['close']) - float(r['open'])) for r in rows]
        efficiency = sum(bodies) / sum(ranges)
        return max(0.0, min(1.0, efficiency))
    except (KeyError, TypeError, ValueError, ZeroDivisionError):
        return 0.0


def select_timeframe(m1_candles, m3_candles, m1_ai: Any, m3_ai: Any, is_otc: bool = False) -> Dict[str, Any]:
    """Escolhe M1 ou M3 por consenso AI + qualidade do candle.

    A escolha é consultiva e fail-closed: empate, dados insuficientes ou
    anomalia alta produzem WAIT, nunca uma ordem.
    """
    threshold = _signal_threshold(is_otc)
    candidates = []
    for tf, candles, ai in (("M1", m1_candles, m1_ai), ("M3", m3_candles, m3_ai)):
        if not candles or ai is None:
            continue
        score = float(getattr(ai, "score", 0) or 0)
        probability = float(getattr(ai, "probability", 0) or 0)
        anomaly = float(getattr(ai, "anomaly_score", 100) or 100)
        quality = _quality(candles)
        composite = score * 0.65 + probability * 100 * 0.20 + quality * 100 * 0.15
        if anomaly > 85:
            composite = -1
        candidates.append({"timeframe": tf, "composite": round(composite, 2), "ai_score": score,
                           "probability": probability, "anomaly": anomaly, "candle_quality": round(quality, 3)})

    if not candidates:
        return {"selected": None, "decision": "WAIT", "reason": "TIMEFRAME_DATA_INSUFFICIENT", "candidates": [], "signal_threshold": threshold}

    candidates.sort(key=lambda x: x["composite"], reverse=True)
    best = candidates[0]
    if best["composite"] < 0 or best["ai_score"] < threshold or best["anomaly"] > 85:
        return {"selected": None, "decision": "WAIT", "reason": "TIMEFRAME_AI_VETO", "candidates": candidates, "signal_threshold": threshold}
    if len(candidates) > 1 and abs(best["composite"] - candidates[1]["composite"]) < 2:
        return {"selected": None, "decision": "WAIT", "reason": "TIMEFRAME_CONSENSUS_TIE", "candidates": candidates, "signal_threshold": threshold}
    return {"selected": best["timeframe"], "decision": "SELECTED", "reason": "AI_SCORE_PROBABILITY_ANOMALY_AND_CANDLE_QUALITY", "candidates": candidates, "signal_threshold": threshold}
